...
void main() {
  ...
  MarvelCharactersResponse sectionInfo = MarvelCharactersResponse(
    offset: 0,
    limit: 20,
    total: 1564,
    count: 20,
  ...
  group('MarvelCharactersResponse.fromMap', () {
    test('map to MarvelCharactersResponse model', () {
      final section = MarvelCharactersResponse.fromMap(map);
      expect(section.offset, 0);
      expect(section.limit, 20);
      expect(section.total, 1564);
      expect(section.count, 20);
      expect(section.characters.length, 2);
    });

    test('throws an exception with an invalid map', () {
      expect(
        () => MarvelCharactersResponse.fromMap(invalidMap),
        throwsA(predicate((e) {
          return e is Exception &&
              e.toString().contains('Error MarvelCharactersResponse.fromMap');
        })),
      );
    });
  });
  ...
  group('SectionInfo.toMap', () {
    test('converts SectionInfo model to map', () {
      ...
      expect(map["offset"], 0);
      expect(map["limit"], 20);
      expect(map["total"], 1564);
      expect(map["count"], 20);
      expect(map["results"].length, 0);
    });
  });
}
